...
export function calculateMacros(
  ...
  const weeklyChangeKg = goalConfig.targetWeightChangePerWeek ?? goalAdjustment.weeklyKg;
  ...
  // Calculate macros
  const protein = calculateProtein(stats, goalConfig.goal);
  ...
  if (weeklyChangeKg < 0) {
    explanation += `To lose ${Math.abs(weeklyChangeKg).toFixed(2)}kg per week, you need a ${Math.abs(Math.round(dailyCalorieAdjustment))} calorie daily deficit. `;
    explanation += `Protein is set high (${protein}g) to preserve muscle during your cut.`;
  } else if (weeklyChangeKg > 0) {
    explanation += `To gain ${weeklyChangeKg.toFixed(2)}kg per week, you need a ${Math.round(dailyCalorieAdjustment)} calorie daily surplus. `;

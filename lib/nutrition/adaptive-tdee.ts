...
function detectLowCalorieOutliers(
  ...
  if (outlierDates.size > 0) {
    ...
    console.log(`[TDEE] Detected ${outlierDates.size} low-calorie outlier days (${extremeCount} extreme < ${extremeOutlierThreshold} cal, ${moderateCount} moderate < ${finalThreshold.toFixed(0)} cal)`);
    console.log(`[TDEE] Threshold calculation: mean=${mean.toFixed(0)} cal, SD=${sd.toFixed(0)} cal, threshold=${finalThreshold.toFixed(0)} cal`);
  }

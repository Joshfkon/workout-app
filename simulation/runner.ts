...
export async function runSimulation(options: RunOptions): Promise<RunResult> {
  ...
  try {
    ...
    for (let sessionIndex = 0; sessionIndex < sessions; sessionIndex++) {
      ...
      // seeded sessions stay `in_progress` for ever and every one of those
      // paths is outside the harness's reach, which makes "the simulation is
      // green" a claim about less than it appears to be.

...
// RPE-based labels for duration exercises (maps to same values)
const DURATION_OPTIONS: RIROption[] = [
  {
    value: 4,
    label: 'Easy',
    subLabel: 'RPE 6',
...
interface RIRSelectorProps {
  ...
  disabled?: boolean;
  /** If true, show RPE-based labels instead of RIR for duration exercises */
 ...
 * For duration exercises, shows RPE-based labels instead
 */
  ...
  return (
    <div className="space-y-2">
      <label className="flex items-center gap-1 text-sm font-medium text-surface-300">
        {isDurationBased ? 'How hard was it?' : 'Reps left in tank?'}
        <InfoTooltip term={isDurationBased ? 'RPE' : 'RIR'} size="sm" />
      </label>
      <div className="grid grid-cols-4 gap-2">
        {(isDurationBased ? DURATION_OPTIONS : RIR_OPTIONS).map((option) => {

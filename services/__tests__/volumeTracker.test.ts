...
import { DEFAULT_VOLUME_LANDMARKS, MUSCLE_GROUPS } from '@/types/schema';
  ...
  it('defines landmarks for every canonical muscle group in every tier', () => {
    for (const tier of tiers) {
      for (const muscle of MUSCLE_GROUPS) {
        const lm = DEFAULT_VOLUME_LANDMARKS[tier][muscle];
...
describe('calculateWeeklyVolume', () => {
  ...
  it('accumulates fractional secondary credit across exercises before flooring', () => {
    ...
    // Correct: accumulate 0.5 + 0.5 = 1.0, floored once -> 1.
  ...
  it('floors leftover fractional secondary credit (does not round up)', () => {
    // Single exercise, 1 working set -> 0.5 triceps credit -> floor -> 0.
    ...
    const result = calculateWeeklyVolume(input);
    expect(result.get('triceps')!.indirectSets).toBe(0);
  });

  it('normalizes muscle-name synonyms and casing', () => {
    // "Quadriceps"/"Hamstring" should map to canonical quads/hamstrings.
    const ex = createMockExercise('Quadriceps', ['Hamstring']);
    const block = createMockBlock(ex.id);
    ...
    expect(result.get('hamstrings')!.indirectSets).toBe(2); // floor(4 * 0.5)
  });
});
...
describe('getVolumeSummary', () => {
  ...
  it('returns 0 average for an empty map (no divide-by-zero)', () => {
    const summary = getVolumeSummary(new Map<string, MuscleVolumeData>());

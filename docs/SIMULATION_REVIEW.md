...
| L4 | Limited deload / mesocycle progression across a run | Sessions are seeded from a template, so the *start* side is not driven — but each one now finishes through the real flow, so the post-session week advance and deload check do run | Follows from L2 |
| ~~L7~~ | ~~Sessions never finished~~ | **CLOSED** — the loop called neither `completeSession` nor the production delete+renumber, so the finish, post-finish settlement, mesocycle updates and deletion semantics were all outside the harness. Both now go through production. The full sweep is unchanged by it (same 32 violations, same guardrail counts), so recorded seeds stay valid. | — |

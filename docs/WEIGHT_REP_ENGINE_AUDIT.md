...
| **5. Session-build fallback** | **#1** (history-blind, schedule-driven block targets) | `startMesocycleSession` now batch-fetches direct history for the day's exercises and derives `target_weight_kg` from the decayed e1RM anchor **through the same live session-start recommender** (`recommendSeedForSlot`: ±10% recent-working-weight clamp, all-sets bump gate, confirmed-regression step-down — so the stored plan can't over-bump off one strong set) — with **no `intensityModifier`** on that path, ending the double-count of the RIR ramp. Deload weeks still apply the modifier (it *is* the deload lightening mechanism). True cold starts keep the `quickWeightEstimate` transfer ladder bit-for-bit (test-verified against the engine output). Warmup protocols now key off the realistic target. |
